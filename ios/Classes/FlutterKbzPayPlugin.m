...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"startPay" isEqualToString:call.method]) {
    ...
    result((data));
  }else if ([@"instantStartPay" isEqualToString:call.method]) {
    NSString *buildInfo = call.arguments[@"build_info"];
    NSString *signKey = call.arguments[@"sign_key"];
    NSString *urlScheme = call.arguments[@"url_scheme"];
    NSString *data = [self instantPay:buildInfo :signKey :urlScheme];
    result((data));
...
- (NSString *)instantPay:(NSString*)buildInfo :(NSString*)signKey :(NSString*)urlScheme{
    NSString *orderString = buildInfo;
    NSLog(@"orderString data %@",orderString);
    NSString *sign = signKey;
    NSLog(@"sign data %@",sign);
    PaymentViewController *vc = [PaymentViewController new];
    [vc startPayWithOrderInfo:orderString signType:@"SHA256" sign:sign appScheme:urlScheme];
    return orderString;
